...
def scalarize_metric(value):
    """Convert tensor metrics to Python floats before logging."""
...
def set_global_seed(seed: int):
    """Seed all local RNGs used by training and rollout batch construction."""
        ...
        for epoch in range(args.epochs_per_rollout_batch):
            ...
            for update_step in range(num_updates_per_epoch):
                ...
                if args.length_norm_type == "mask_dapo":
                    ...
                    acc_steps_to_pass = args.gradient_accumulation_steps
                log_accum_denominator = (
                    1 if args.length_norm_type == "mask_dapo"
                ...
                for micro_step in range(args.gradient_accumulation_steps):
                    ...
                    batch_clip_frac += scalarize_metric(loss_meta.get('clip_fraction', 0))
                    batch_avg_response_entropy += avg_res_entropy
                    batch_avg_global_entropy += avg_global_entropy
                    batch_ratio_mean += scalarize_metric(loss_meta.get('ratio_mean', 0))
                ...
                if global_step % 5 == 0:
                    wandb.log({
                        "train/loss": batch_loss / log_accum_denominator,
