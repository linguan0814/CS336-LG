...
def grpo_microbatch_train_step(
    ...
    # The loss itself is masked above, so keep GRPO diagnostics on the same
    # response-token surface instead of including prompt/pad positions.
    if old_log_probs is not None and loss_type in {"grpo_clip", "grpo_no_clip"}:
        with torch.no_grad():
            mask = response_mask.to(dtype=torch.bool, device=policy_log_probs.device)
            ratio = torch.exp(policy_log_probs.detach() - old_log_probs.detach())

            if mask.any():
                masked_ratio = ratio[mask]
                metadata["ratio_mean"] = masked_ratio.mean()
                metadata["ratio_max"] = masked_ratio.max()
                metadata["ratio_min"] = masked_ratio.min()

                if cliprange is not None:
                    ratio_clipped = torch.clamp(ratio, 1.0 - cliprange, 1.0 + cliprange)
                    surr1 = ratio * advantages
                    surr2 = ratio_clipped * advantages
                    clipped_mask = (surr2 < surr1)[mask]
                    metadata["clip_fraction"] = clipped_mask.to(torch.float32).mean()
