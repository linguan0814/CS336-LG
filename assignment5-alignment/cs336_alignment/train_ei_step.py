...
def run_expert_iteration(args):
    ...
    for ei_step in range(args.n_ei_steps):
        ...
        for i, output in enumerate(outputs):
            current_goal = batch_db[i]['gold']
            success_flag = 0
            for candidate in output.outputs:

                if reward_fn(candidate.text, current_goal)['reward'] == 1.0:
                    success_flag = 1
        ...
        for _ in pbar:
            ...
            for _ in range(grad_accum_steps):
                ...
                with amp_ctx:
                    ...
                    acc_loss += loss.item() * grad_accum_steps #还原原始 loss 大小，用于日志记录
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no_save_checkpoints", action="store_true", help="只记录 W&B，不保存每轮 EI 权重")
