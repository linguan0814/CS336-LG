...
def parse_args():
    ...
    parser.add_argument("--wandb_dir", type=str, default="artifacts/wandb", help="Local directory for Wandb files")
...
def init_wandb(args, device, data_meta):
    ...
    wandb.define_metric("model/*", step_metric="iteration")
    print(f"Wandb initialized: project={args.wandb_project}, group={group}, run={wandb.run.name}")
...
def main():
    ...
    for iter_num in pbar:
        ...
        # Non-finite gradient check before gradient clipping
        bad_grad = False
        bad_grad_name = None
        ...
        if iter_num % args.save_intervals == 0 and iter_num > 0:
            ...
            save_checkpoint(model, optimizer, iter_num, checkpoint_path, model_config=model_config)
    ...
    # If training stopped early because of non-finite loss/grad, this records the actual completed iteration.
    final_checkpoint_path = os.path.join(args.save_ckp_path, f"checkpoint_final_{completed_iter}.pt")
