...
def _make_attn_inputs_with_shape(batch_size, n_queries, n_keys, D, device=None):
    torch.random.manual_seed(0)
    q = torch.randn(batch_size, n_queries, D, device=device, requires_grad=True)
    k = torch.randn(batch_size, n_keys, D, device=device, requires_grad=True)
    v = torch.randn(batch_size, n_keys, D, device=device, requires_grad=True)
...
def _test_flash_forward_pass(impl, device="cpu", is_causal=False):
    ...
    torch.testing.assert_close(lse, l_ref, rtol=1e-2, atol=1e-2)
...
def test_flash_backward_triton_non_tile_shapes(is_causal, D):
    q, k, v, do = _make_attn_inputs_with_shape(2, 127, 129, D, device="cuda")
    q_ref, k_ref, v_ref, _ = _make_attn_inputs_with_shape(2, 127, 129, D, device="cuda")

    _attention_and_lse(q_ref, k_ref, v_ref, is_causal)[0].backward(do)
    get_flashattention_autograd_function_triton().apply(q, k, v, is_causal).backward(do)

    torch.testing.assert_close(q_ref.grad, q.grad, rtol=1e-2, atol=1e-2)
    torch.testing.assert_close(k_ref.grad, k.grad, rtol=1e-2, atol=1e-2)
    torch.testing.assert_close(v_ref.grad, v.grad, rtol=1e-2, atol=1e-2)
