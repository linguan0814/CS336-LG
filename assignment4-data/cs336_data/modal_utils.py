...
def build_image(*, include_tests: bool = False) -> modal.Image:
    ...
    # These local instruction files are optional and intentionally ignored by
    # the public release.  Include them only when running from the original
    # course workspace.
    for local_file in ("AGENTS.md", "CLAUDE.md"):
        if Path(local_file).is_file():
            image = image.add_local_file(local_file, f"/root/{local_file}")
    if include_tests:
